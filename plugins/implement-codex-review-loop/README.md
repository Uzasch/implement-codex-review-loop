...
Implement a PRD or a set of issues, then harden the result in a Claude↔Codex review loop: Claude
implements and self-reviews, Codex reviews independently in one persistent session, Claude triages
every finding against the ADRs and the originating issue, fixes what it accepts, and answers back.
Repeats until Codex approves or the round cap is hit.
